...
describe('TagsModalComponent', () => {
  ...
  function getSaveButton(): DebugElement {
    return fixture.debugElement.query(By.css('.tagsModal-save'));
  }
  ...
  describe('on initialisation', () => {
    ...
    describe('when the save tags button is clicked', () => {
      let saveConfigPromise: TestPromise<void>;

      beforeEach(() => {
        saveConfigPromise = new TestPromise<void>();
        (dependencies.boardConfigStoreService
          .saveConfig as jasmine.Spy).and.returnValue(saveConfigPromise);
        getSaveButton().nativeElement.click();
      });

      it('should save the tags config', () => {
        expect(
          dependencies.boardConfigStoreService.saveConfig
        ).toHaveBeenCalledWith({ tags: allTags });
      });

      describe('when the tags have saved', () => {
        beforeEach(async(() => {
          saveConfigPromise.resolve();
        }));
